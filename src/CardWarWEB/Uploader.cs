...
public class Uploader
{
  ...
  */
    public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
    {
        ...
        uploadpath = Server.MapPath(pathbase);//获取文件上传路径

        try
        {
            uploadFile = cxt.Request.Form.Files[0];
            var parsedContentDisposition = ContentDispositionHeaderValue.Parse(uploadFile.ContentDisposition);
 ...
 */
    public Hashtable upScrawl(HttpContext cxt, string pathbase, string tmppath, string base64Data)
    {
        ...
        {
            deleteFolder(Server.MapPath(tmppath));
        }
        return getUploadInfo();
...
*/
    public string getOtherInfo(HttpContext cxt, string field)
    {
        string info = null;
        try {
            if (!string.IsNullOrEmpty(cxt.Request.Form[field]))
            {
                string s = cxt.Request.Form[field];
                info = field == "fileName" ? s.Split(',')[1] : s;
            }
        }
        catch
        {
